...
export class TicTacToe extends Scene {
    ...
    resetGame() {
        ...
        this.grid.forEach((row) =>
            row.forEach((cell) => cell.setFillStyle(0xffffff)),
        );
    ...
    endGame() {
        this.grid.forEach((row) =>
            row.forEach((cell) => cell.disableInteractive()),
        );
    ...
    checkWin(): boolean {
        ...
        for (let i = 0; i < 3; i++) {
            if (
                b[i][0] === this.currentPlayer &&
                b[i][1] === this.currentPlayer &&
                b[i][2] === this.currentPlayer
            )
                return true;
            if (
                b[0][i] === this.currentPlayer &&
                b[1][i] === this.currentPlayer &&
                b[2][i] === this.currentPlayer
            )
                return true;
        }
        if (
            b[0][0] === this.currentPlayer &&
            b[1][1] === this.currentPlayer &&
            b[2][2] === this.currentPlayer
        )
            return true;
        if (
            b[0][2] === this.currentPlayer &&
            b[1][1] === this.currentPlayer &&
            b[2][0] === this.currentPlayer
        )
            return true;
        return false;
    }
